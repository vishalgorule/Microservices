...
public class KafkaAdminClient {
            ...
            return webClient
                    ...
                    .exchangeToMono(response -> {
                        if (response.statusCode().is2xxSuccessful()) {
                            LOG.info("Successful response from schema registry");
                            return Mono.just(response.statusCode());
                        } else {
                            LOG.info("Failed response from schema registry");
                            return Mono.just(HttpStatus.SERVICE_UNAVAILABLE);
                        }
                    }).block();
        } catch (Exception e) {
            LOG.info("Exception response from schema registry");
    ...
    private Collection<TopicListing> doGetTopics(RetryContext retryContext) throws ExecutionException, InterruptedException {
        LOG.info("Reading kafka topics {}, attempts {}",
